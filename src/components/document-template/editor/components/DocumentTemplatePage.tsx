...
import { DocumentTemplatePageActions } from './DocumentTemplatePageActions';
import { DocumentTemplatePageBody } from './DocumentTemplatePageBody';
import { DocumentTemplatePageFooter } from './DocumentTemplatePageFooter';
import { DocumentTemplatePageHeader } from './DocumentTemplatePageHeader';
...
export function DocumentTemplatePage({
  ...
  return (
                    <div
                      ...
                      style={pageScaleViewportStyle()}
                    >
                      <div
                        className={`document-page document-template-page bg-background relative grid overflow-hidden shadow-[0_18px_50px_rgba(15,23,42,0.18)] ring-1 ring-black/5 ${
                          pageIndex === 0 ? 'z-10' : 'z-0'
                        }`}
                        style={{
                          width: A4_EDITOR_WIDTH_PX,
                          minWidth: A4_EDITOR_WIDTH_PX,
                          maxWidth: A4_EDITOR_WIDTH_PX,
                          height: A4_EDITOR_HEIGHT_PX,
                          minHeight: A4_EDITOR_HEIGHT_PX,
                          maxHeight: A4_EDITOR_HEIGHT_PX,
                          overflow: 'hidden',
                          position: 'relative',
                          ...
                          gridTemplateRows:
                            DISABLE_HEADER_FOOTER_FOR_LAYOUT_DEBUG
                              ? 'auto'
                              : `${headerHeightPx}px ${bodyAreaHeightPx}px ${footerHeightPx}px`,
                          ...pageScaleStyle(),
                        }}
                      >
                        ...
                        <DocumentTemplatePageActions
                          controller={controller}
                          pageIndex={pageIndex}
                        />
                        <DocumentTemplatePageHeader
                          controller={controller}
                          pageIndex={pageIndex}
                        />

                        <DocumentTemplatePageBody
                          controller={controller}
                          pageIndex={pageIndex}
                        />

                        <DocumentTemplatePageFooter
                          controller={controller}
                          pageIndex={pageIndex}
                        />
                      </div>
