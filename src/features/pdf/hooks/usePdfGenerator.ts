...
const generatePdfApi = async (
  ...
  const filename =
    parsedFilename || generateFallbackFilename(request.einsatzId);
...
export function usePdfGenerator() {
  ...
  const mutation = useMutation({
    ...
    onMutate: (variables) => {
      ...
      console.log(
        "Generating PDF with key:",
        pdfQueryKeys.generate(variables.einsatzId)
      );
    },
  ...
  const generatePdf = useCallback(
    async (
      ...
      try {
        ...
        return console.error("Error in generatePdf:", error), null;
      }
    },
    [mutation]
  );
