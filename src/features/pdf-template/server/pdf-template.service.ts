...
import { buildBookingConfirmationPdfInput } from '../lib/pdf-template-inputs';
import {
...
import {
  ...
  getPdfTemplateSettingsPath,
  getPdfTemplatesSettingsPath,
} from '../lib/pdf-template-routes';
import {
...
import { applyFooterToTemplate } from '../lib/pdf-template-footer';
import { applyImageBindingsToTemplate } from '../lib/pdf-template-image';
...
function revalidatePdfTemplatePaths(
  organizationId: string,
  templateId?: string
): void {
  revalidatePath(getPdfTemplatesSettingsPath(organizationId));
  revalidatePath(`/settings/org/${organizationId}`);

  if (templateId) {
    revalidatePath(getPdfTemplateSettingsPath(organizationId, templateId));
    revalidatePath(getEditPdfTemplateSettingsPath(organizationId, templateId));
  }
}
...
export async function createPdfTemplate(args: {
  ...
  if (args.isDefault) {
    ...
    revalidatePdfTemplatePaths(created.organizationId, created.id);
...
export async function updatePdfTemplate(args: {
  ...
  revalidatePdfTemplatePaths(current.organizationId, args.id);
...
export async function setDefaultPdfTemplate(id: string): Promise<PdfTemplateRecord> {
  ...
  revalidatePdfTemplatePaths(targetRow.organizationId, id);
  return mapRowToRecord(updated);
}
