...
export function useCreateEinsatz(
    ...
    return useMutation({
        ...
        onMutate: async (event) => {
            ...
            const optimisticVars: EinsatzCreate = { ...event };
...
export function useUpdateEinsatz(
    ...
    return useMutation({
        ...
        onError: (error, _vars, ctx) => {
            queryClient.setQueryData(queryKey, ctx?.previous);
            toast.error(`${einsatzSingular} konnte nicht aktualisiert werden: ${error}`);
        },
        onSuccess: (_data, vars) => {
            const title = 'org_id' in vars ? vars.title : vars.title;
            toast.success(`${einsatzSingular} '${title}' wurde aktualisiert.`);
        },
        onSettled: (data, _error) => {
            queryClient.invalidateQueries({
                queryKey: queryKeys.einsaetze(activeOrgId ?? data?.org_id ?? ''),
            });
        },
    });
}
