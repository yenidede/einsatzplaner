...
interface UseEinsatzResult {
  ...
  updateEinsatz: (id: string, data: Partial<EinsatzFormData>) => Promise<Einsatz | null>;
...
export function useEinsatz(): UseEinsatzResult {
  ...
  const [currentFilter, setCurrentFilter] = useState<EinsatzFilter | undefined>();

  const fetchEinsaetze = useCallback(async (filter?: EinsatzFilter, page: number = 1) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (filter) {
        ...
        if (filter.systemStatus) params.append('systemStatus', filter.systemStatus);
        if (filter.datum?.from) params.append('dateFrom', filter.datum.from.toISOString());
        if (filter.datum?.to) params.append('dateTo', filter.datum.to.toISOString());
      }
  ...
  const createEinsatz = useCallback(async (data: EinsatzFormData): Promise<Einsatz> => {
  ...
  const updateEinsatz = useCallback(async (id: string, data: Partial<EinsatzFormData>): Promise<Einsatz | null> => {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Fehler beim Aktualisieren des Einsatzes');
    }

    const updatedEinsatz = await response.json();
    
    // Update local state
    setEinsaetze(prev => prev.map(e => e._id === id ? updatedEinsatz : e));
...
interface UseAutosaveResult {
  saveAutosave: (einsatzId: string | undefined, formData: Partial<EinsatzFormData>) => Promise<void>;
...
export function useAutosave(): UseAutosaveResult {
  const saveAutosave = useCallback(async (einsatzId: string | undefined, formData: Partial<EinsatzFormData>) => {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Fehler beim Speichern der Autosave-Daten');
    }
  }, []);

  const getAutosave = useCallback(async (einsatzId: string | undefined): Promise<AutosaveData | null> => {
    const params = new URLSearchParams();
