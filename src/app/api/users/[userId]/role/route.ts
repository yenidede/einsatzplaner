...
export async function PUT(request: NextRequest, {params}: RouteParams){
    try {
        ...
        const validRoles = ["OV", "EV", "Einsatzverwaltung", "Organisationsverwaltung", "Helfer"];
        ...
        const currentUserInOrganisation = await prisma.user_organization_role.findFirst(
            {
                where: {
        ...
        const targetUserOrganisation = await prisma.user_organization_role.findFirst(
            {
                where: {
                    user_id: session.user.id,
                    org_id: organizationId
        ...
        if(action === "add"){
            const hasRole = currentUserRoles.some((uor: any) => uor.role.abbreviation === role);
            ...
            const hasRoleToRemove = currentUserRoles.find((uor) => uor.role.abbreviation === role);
