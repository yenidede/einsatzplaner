...
export const useAlertDialog = () => {
  ...
  const showDialog = useCallback((options: AlertDialogOptions): Promise<AlertDialogResult> => {
  ...
  const handleClose = useCallback((result: AlertDialogResult) => {
    if (state.resolve) {
  ...
  const AlertDialogComponent = state.isOpen && state.options ? React.createElement(
    AlertDialog,
    { open: state.isOpen, onOpenChange: handleOpenChange },
    React.createElement(
      AlertDialogContent,
      null,
      React.createElement(
        ...
        React.createElement(
          AlertDialogTitle,
          { 
            className: state.options.variant === 'destructive' ? 'text-red-600' : '' 
          },
          state.options.title
        ),
        React.createElement(
...
export const useConfirmDialog = () => {
  ...
  const confirm = useCallback(
    (title: string, description: string, confirmText = 'Bestätigen', cancelText = 'Abbrechen') => {
      return showDialog({
        title,
        description,
        confirmText,
  ...
  const confirmDestructive = useCallback(
    (title: string, description: string, confirmText = 'Löschen', cancelText = 'Abbrechen') => {
      return showDialog({
        title,
        description,
        confirmText,
...
export const useGlobalAlertDialog = () => {
  ...
  if (!context) {
    throw new Error('useGlobalAlertDialog must be used within an AlertDialogProvider')
  }
  return context
}
