...
- Do not edit `prisma/schema.prisma`. If a task requires a data-model change, explain the required database change to the user, why it is needed, and where the current model is limiting the implementation. You may suggest proper SQL Syntax. If edits are highly suggested or necessary, block the further execution and ask the user for an Action.
